(function () {
  ...
  function href(route) {
    return "https://wa.me/" + WA + "?text=" + encodeURIComponent(msg(route));
  }
  document.querySelectorAll("[data-wa-route]").forEach((a) => {
    a.href = href(a.dataset.waRoute || "");
  });
  ...
  const langs = ["zh-CN", "en", "fr", "de", "it", "ko", "pt", "ru", "es", "vi"];
  const labels = {
    "zh-CN": "Chinese",
    en: "English",
    fr: "French",
  ...
  if (selects.length > 1 && selects[1].options.length === 0) {
    langs.forEach((l) => selects[1].append(new Option(labels[l], l)));
  }
  function detect() {
  ...
  function applyLang(lang) {
    const dict = (window.KT_I18N && window.KT_I18N[lang]) || window.KT_I18N.en;
  ...
  selects.forEach((s) =>
    s.addEventListener("change", (e) => applyLang(e.target.value)),
  );
  if (window.KT_I18N) applyLang(detect());
  const modal = document.getElementById("telegram-modal");
  document.querySelectorAll(".telegram-open").forEach((btn) =>
    btn.addEventListener("click", () => {
      if (modal) modal.hidden = false;
    }),
  );
  document.querySelectorAll(".telegram-close").forEach((btn) =>
    btn.addEventListener("click", () => {
  ...
  document.querySelectorAll(".copy-telegram").forEach((btn) =>
    btn.addEventListener("click", async () => {
      const status = document.querySelector(".copy-status");
      try {
        await navigator.clipboard.writeText(btn.dataset.copy || TG);
        if (status) status.textContent = "Copied Telegram number.";
      } catch (e) {
